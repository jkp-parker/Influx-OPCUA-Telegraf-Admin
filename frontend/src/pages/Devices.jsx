...
import {
  Plus, Pencil, Trash2, Wifi, ChevronRight,
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2 flex items-center gap-2">
          <input type="checkbox" id="enabled" checked={form.enabled} onChange={e => set('enabled', e.target.checked)} className="rounded border-gray-600 bg-gray-800 text-blue-500" />
          <label htmlFor="enabled" className="text-sm text-gray-300">Device enabled</label>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-100">OPC UA Devices</h1>
          <p className="text-sm text-gray-400 mt-1">Configure your OPC UA data sources</p>
        </div>
      ...
      {devices.length === 0 ? (
        <div className="card p-12 text-center text-gray-500">
          <ServerIcon className="mx-auto mb-3 text-gray-600" size={40} />
          <p className="font-medium text-gray-300">No devices configured</p>
                  ...
                  return (
                    <tr key={d.id} className="hover:bg-gray-800/50">
                      <td className="table-td">
                        <Link to={`/devices/${d.id}`} className="font-semibold text-blue-400 hover:text-blue-300 flex items-center gap-1">
                          {d.name} <ChevronRight size={14} />
                        </Link>
                      </td>
                      <td className="table-td font-mono text-xs text-gray-400 truncate max-w-xs" title={d.endpoint_url}>{d.endpoint_url}</td>
                      <td className="table-td text-xs text-gray-400">{d.security_policy}</td>
                      <td className="table-td text-center">
                        <span className="flex items-center justify-center gap-1 text-sm">
                          <Tag size={12} className="text-gray-500" />
                          <span className="font-semibold">{d.enabled_tag_count}</span>
                          <span className="text-gray-500">/ {d.tag_count}</span>
                        </span>
                      </td>
                      <td className="table-td text-xs text-gray-400">{d.influxdb_name || <span className="text-gray-600">system default</span>}</td>
                      <td className="table-td">
                        {tr ? (
                      ...
                      <td className="table-td">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={() => handleDelete(d.id)} className="btn-ghost py-1 px-2 text-red-400 hover:bg-red-900/30" title="Delete">
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </td>
...
function ServerIcon({ size, ...props }) {
  return (
