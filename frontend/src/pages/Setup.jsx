      ...
      <div className="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="bg-blue-600 px-8 py-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white text-xl font-bold">FluxForge</span>
          </div>
          ...
          <div className="flex items-center gap-0">
            {STEPS.map((label, i) => (
              <div key={i} className="flex items-center">
                <div className={`flex items-center gap-1.5 ${i <= step ? 'text-blue-400' : 'text-gray-500'}`}>
                ...
                {i < STEPS.length - 1 && (
                  <div className={`w-8 h-0.5 mx-2 ${i < step ? 'bg-blue-600' : 'bg-gray-700'}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="px-8 py-6 min-h-64">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-100">InfluxDB Connection</h2>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-100">Telegraf Settings</h2>
              <p className="text-sm text-gray-400">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  ...
                  <code className="ml-1 bg-gray-800 px-1 rounded text-xs text-blue-400">systemctl reload telegraf</code>,
                  <code className="ml-1 bg-gray-800 px-1 rounded text-xs text-blue-400">docker restart telegraf</code>
                </p>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-100">Setup Complete</h2>
              <p className="text-gray-400">
                Your portal is configured and ready to use. You can update any of these settings
                later from the <strong className="text-gray-200">Administration</strong> page.
              </p>
              <div className="bg-gray-800 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-400">Portal Name</span><span className="font-medium text-gray-200">{form.app_title}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">InfluxDB URL</span><span className="font-medium text-gray-200">{form.influxdb_url || '(not set)'}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Organisation</span><span className="font-medium text-gray-200">{form.influxdb_org || '(not set)'}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Default Bucket</span><span className="font-medium text-gray-200">{form.influxdb_default_bucket || '(not set)'}</span></div>
                <div className="flex justify-between"><span className="text-gray-400">Config Path</span><span className="font-medium text-gray-200">{form.telegraf_config_path}</span></div>
              </div>
        ...
        {/* Footer nav */}
        <div className="px-8 py-4 bg-gray-800 border-t border-gray-700 flex justify-between">
          <button
